...
const News: NextPage = () => {
    ...
    useEffect( () => {
        (async () => {
            const wakuGlobalContext: WakuContext = await initWakuContext({onRootReceived: onMessageReceived, onLikeReceived: onLikeMessageReceived});
            ...
            <div className="flex items-center flex-col flex-grow pt-10">
                {
                    newsList.map((theNews) => {
                        return <NewsItem {...theNews} key={`${theNews.id}-${Date.now()}`} onLike={() => onLike(theNews.id)} onDisLike={() => onDisLike(theNews.id)} />
                    })
                }
            </div>
        </>
